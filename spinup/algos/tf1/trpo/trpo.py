...
class GAEBuffer:
  ...
  def __init__(self, obs_dim, act_dim, size, info_shapes, gamma=0.99, lam=0.95):
    ...
    self.info_bufs = {k: np.zeros([size] + list(v), dtype=np.float32) for k, v
                      in info_shapes.items()}
...
def trpo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
  ...
  pi, logp, logp_pi, info, info_phs, d_kl, v = actor_critic(x_ph, a_ph,
                                                            **ac_kwargs)

  # Need all placeholders in *this* order later (to zip with data from buffer)
  all_phs = [x_ph, a_ph, adv_ph, ret_ph,
             logp_old_ph] + core.values_as_sorted_list(info_phs)
  ...
  buf = GAEBuffer(obs_dim, act_dim, local_steps_per_epoch, info_shapes, gamma,
                  lam)
  ...
  for epoch in range(epochs):
    for t in range(local_steps_per_epoch):
      ...
      a, v_t, logp_t, info_t = agent_outs[0][0], agent_outs[1], agent_outs[
        2], agent_outs[3:]
